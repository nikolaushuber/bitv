...
(** This module implements bit vectors, as an abstract datatype [t].
    ...
    (*@ function map2 (f : 'a -> 'b -> 'c) (s1 : 'a sequence) (s2 : 'b sequence) : 'c sequence =
    ...
    (*@ function op_xor (a : bool) (b : bool) : bool =
        ...
        (*@ r = sub v start len
            ...
            ensures r.size = len
            ensures r.bits = if len = 0 then Sequence.empty else v.bits[start..start+len-1] *)
        ...
        (*@ blit v1 o1 v2 o2 len
            ...
            ensures v2.bits =
                ...
                else old (v2.bits[..o2-1]) ++ v1.bits[o1..o1+len-1] ++ v2.bits[o2+len..] *)
        ...
        (*@ r = shiftl v n
            ...
            ensures r.bits =
                if n = 0 then v.bits
                    ...
                    if n < v.size then Sequence.init n (fun _ -> false) ++ v.bits[..(v.size - n - 1)]
        ...
        (*@ r = rotatel v n
            ...
            ensures r.bits =
                if n = 0 then v.bits
                else if n > 0 then v.bits[(v.size - (mod n v.size))..] ++ v.bits[..(v.size - (mod n v.size) - 1)]
        ...
        (*@ r = rotater v n
            ...
            ensures r.bits =
                if n = 0 then v.bits
                else if n > 0 then v.bits[(mod n v.size)..] ++ v.bits[..((mod n v.size) - 1)]
                else v.bits[(v.size - (mod (-n) v.size))..] ++ v.bits[..(v.size - (mod (-n) v.size) - 1)] *)
